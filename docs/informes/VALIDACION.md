...
| qué | cuánto |
|-----|--------|
| hallazgos numerados en `FINDINGS.md` | **306** |
| secciones, contando las sub-numeradas con letra (140b, 220f…) | **318** |
| experimentos ejecutables en `cmd/` | **244** |
| láminas en `galeria/laminas/` | **147** |
| sonidos en `galeria/sonidos/` | **8** |
| paradas del museo | **275** |
| máximas del capitán, verificadas una por una | **19** |

Para contarlos:

```bash
rg -o "^## Finding [0-9]+" docs/registro/FINDINGS.md | rg -o "[0-9]+" | sort -n | uniq | wc -l
...
```bash
rg -o "^## Finding [0-9]+" docs/registro/FINDINGS.md | sort | uniq -c | grep -v " 1 "
```
...
- **`galeria/index.html`** — las 147 láminas. Cada una la dibuja su propio
  experimento: corra el comando y compare el SVG.
- **`galeria/museo.html`** — 275 paradas en criollo. Cada pieza cierra con su
...
- [FINDINGS.md](../registro/FINDINGS.md) — los 280 hallazgos numerados, en inglés, con sus
  números, sus comandos y sus hipótesis muertas.
- [HALLAZGOS-ES.md](../registro/HALLAZGOS-ES.md) — el mismo registro en castellano.
- [BITACORA-NOCTURNA.md](../registro/BITACORA-NOCTURNA.md) — el diario de a bordo, día por
