// Command encore seats any prime-modulus tribe in the orchestra.
//
// The earlier radios hard-coded their characters. This one builds the
// quadratic character of ANY odd prime modulus q on demand, via Euler's
// criterion: χ(n) = n^((q−1)/2) mod q, read as +1, −1 or 0. One flag, one new
// dial — the orchestra's seating is now unbounded.
//
// Usage:
//
//	go run ./cmd/encore [-mods 11,13] [-limit N]
package main
...
// legendre builds the quadratic character mod q.
...
func main() {
	mods := flag.String("mods", "11,13", "comma-separated odd prime moduli")
	...
	freqs := []float64{}
	for f := 1.0; f <= 30.0; f += 0.005 {
		freqs = append(freqs, f)
	}
	ps := primes.Sieve(*limit)

	for _, tok := range strings.Split(*mods, ",") {
		q, err := strconv.Atoi(strings.TrimSpace(tok))
		if err != nil || q < 3 || q%2 == 0 {
			fmt.Fprintf(os.Stderr, "skipping invalid modulus %q\n", tok)
			continue
		}
		w := legendre(q)
		...
		fmt.Printf("RADIO mod %d — L(s, χ_%d)\n  stations:", q, q)
		for _, k := range topPeaks(freqs, pw, 6) {
			fmt.Printf("  %.4f", k.f)
		}
		fmt.Println()
	}
